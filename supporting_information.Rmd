...
We collected shrubs from the Walker Bay field reasearch station, which is located on the Kent Penninsula (Figure 1). The climate data came from (1) a weather station operational at Walker Bay for ~5 years, (2) Environment Canada weather data from Cambridge Bay, (3) Environment Canada weather data from Byron Bay, and (4) Environment Canada weather data from Kugluktuk (Figure 1). Byron Bay is closest to Walker Bay, but has limited climate data and a different vegatation community (D. Morris, personal observation). We expected \textit{a priori} that Cambridge Bay weather data would be the best match for Walker Bay.
...
A1 = c("June", 2010, 17, "We randomly sampled one shrub \n from 12 lemming sampling grids, \n one shrubs from four quardats of a predator field exclosure, \n and one shrub from a \n snap-trap transect.", "Krebs et al., 2012; Dupuch et al., 2014")
A2 = c("June", 2011, 41, "We randomly sampled forty shrubs were sampled at random four transects (500m long and 100m apart).\n  We added one shrub that \n was much older than the others to extend our chronology.", "No currently published work")
A3 = c("June", 2015, 12, "We randomly sampled one shrub from the same 12 lemming \n sampling grids that were sampled in 2010.", "Dupuch et al., 2014")
...
knitr::kable(AA, caption = "Sampling regime for the shrubs using in our dendrochronological analysis. N is the number of shrub branches sampled, each from an individual shrub.")
```
...
```{r, echo=F,message = F, fig.width=10, fig.height=10,fig.cap="The trends in the relationship between July temperature and growth over time using an 11-year moving window centered on the year displayed on the x-axis. The trends are the same for ring width and ring area."}
