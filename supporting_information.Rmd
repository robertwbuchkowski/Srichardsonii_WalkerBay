...
shrubcompare %>% select(Year, WalkerBay, CambridgeBay, Kugluktuk) %>% gather(-Year, key=Chron, value= Ring) %>% filter(!is.na(Ring)) %>%
  left_join(
    tibble(
      ...
      Chron2 = c("italic(Salix~richardsonii)~at~Walker~Bay","italic(Salix~alaxensis)~at~Cambridge~Bay","italic(Picea~marinan)~at~Kugluktuk")
    )
  ) %>%
  mutate(Chron = Chron2) %>%
  select(-Chron2) %>%
  ggplot(aes(Year, Ring, group=Chron, color=Chron)) + geom_line()+ theme_classic() +
  annotate("text", x=1950,                                                                    y=c(2.2,2.0, 1.8),                                                                 label=c("italic(Salix~richardsonii)~at~Walker~Bay~versus~italic(Salix~alaxensis)~at~Cambridge~Bay: r==0.19",                                          "italic(Salix~richardsonii)~at~Walker~Bay~versus~italic(Picea~marinan)~at~Kugluktuk: r==0.003",
                  "italic(Picea~marinan)~at~Kugluktuk~versus~italic(Salix~alaxensis)~at~Cambridge~Bay: r==0.04"), parse = T) +
...
```{r, echo=F,message = F, fig.width=10, fig.height=10,fig.cap="The relationship between the ring width index (RWI) and May, July, and August total precipitation from 1996 to 2010. These three monthly precipitation variables were significant in a response function analysis."}
