...
subtitle: Warmer temperatures promote shrub radial growth but not cover in the Central Canadian Arctic
output:
...
```{r, echo=F,message = F, fig.width=10, fig.height=5,fig.cap="The linear model coefficient and significance for all possible 14-year time windows in our chronology related basal area increment to year. The analysis shows variable significance over time which is closely aligned with the interannual variability present in that 14-year window (See main text figure). The triangle indicates the coefficient calculated from the 14-year period that matches the shrub cover data."}
read_csv("DataOut/outmove.csv") %>% ggplot(aes(x = start, y = coeff))+ geom_hline(yintercept = 0, lty = 2) + geom_line() + geom_point(aes(col = sig, shape = focus,size = sig)) + theme_classic() + xlab("Start of 14-yr window") + ylab("Coefficient (Year)") +
  scale_color_manual(values = c("blue", "orange"), name = "Significant?") +
  scale_shape_manual(values = c(19, 2),guide = F) + 
  scale_size_manual(values = c(2, 3),guide = F)
```
