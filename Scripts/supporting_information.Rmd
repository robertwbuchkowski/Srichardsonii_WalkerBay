...
# Shrub Data
chrono <- shrubs <- read_csv("Data/shrubs.csv")
...
coppermine <- read_csv("Data/coppermine1.csv") %>% 
  ...
  bind_rows(read_csv("~/Documents/ArcticShrubs/ArcticShrubs/coppermine2.csv") %>% 
              select(Year, Month, MeanT = `Mean Temp (°C)`, TotP =`Total Precip (mm)`) %>%
              filter(!is.na(MeanT))
...
coppermine2 = read.csv("coppermine_f.csv")

respcopper = dcc(chrono = chrono, clim = coppermine2)
...
respbryon = dcc(chrono = chrono, clim = bryonbay2)
...
There was enough data for Byron Bay to repeat the response function analysis comparing Walker Bay shrubs to Byron Bay climate records. No significant relationships were observed (Figure 7).
...
```{r, echo=F,message = F, fig.width=10, fig.height=8,fig.cap="No significant response functions for Byron Bay. Lower case letter indicate months in the year before shrub growth, while capital letters indicate months in the year of shrub growth. MeanT = Mean Temperature; TotP = Total Precipitation"}
dcplot(respbryon)
```
...
rawdata<- read_csv("raw_ring_data_June2019.csv")

rwdu <- read_csv("final_chron_used_mod.csv")
